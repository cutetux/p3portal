// p3portal.org
// PROJ-65: Slim-Modal für Announcement-Details im Hub
import { useTranslation } from 'react-i18next'
import { SEVERITY_DOT } from '../severity'

export default function AnnouncementDetailModalSlim({ item, onClose }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-portal-border">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-portal-text">
              {t('notifications.detail_announcement')}
            </h2>
          </div>
        ...
        <div className="p-5 space-y-3">
          <p className="text-sm text-portal-text leading-relaxed">{item.title}</p>
          {item.summary && (
            <p className="text-xs text-portal-text2 leading-relaxed">{item.summary}</p>
          )}
