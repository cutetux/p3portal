...
| `admin` | `portal-admin@pve` | operator perms + `VM.Allocate`, `VM.Clone`, **`VM.Migrate`**, `VM.Backup`, `VM.Config.{CPU,Memory,Disk,Network,HWType,Options,Cloudinit,CDROM}`, `Datastore.Allocate`, `Datastore.AllocateSpace`, **`Datastore.AllocateTemplate`**, `SDN.Use`, **`SDN.Allocate`**, **`Sys.Audit`**, **`Sys.Modify`**, **`Sys.AccessNetwork`** |
...
pveum role add PortalAdmin \
  --privs "VM.Audit,VM.GuestAgent.Audit,VM.PowerMgmt,VM.Snapshot,VM.Allocate,VM.Clone,VM.Migrate,VM.Backup,\
VM.Config.CPU,VM.Config.Memory,VM.Config.Disk,VM.Config.Network,\
VM.Config.HWType,VM.Config.Options,VM.Config.Cloudinit,VM.Config.CDROM,\
Datastore.Allocate,Datastore.AllocateSpace,Datastore.AllocateTemplate,Datastore.Audit,SDN.Use,SDN.Allocate,Pool.Audit,\
Sys.Audit,Sys.Modify,Sys.AccessNetwork"
...
pveum role modify PortalAdmin    --privs "VM.Audit,VM.GuestAgent.Audit,VM.PowerMgmt,VM.Snapshot,VM.Allocate,VM.Clone,VM.Migrate,VM.Backup,VM.Config.CPU,VM.Config.Memory,VM.Config.Disk,VM.Config.Network,VM.Config.HWType,VM.Config.Options,VM.Config.Cloudinit,VM.Config.CDROM,Datastore.Allocate,Datastore.AllocateSpace,Datastore.AllocateTemplate,Datastore.Audit,SDN.Use,SDN.Allocate,Pool.Audit,Sys.Audit,Sys.Modify,Sys.AccessNetwork"
```
...
| `Permission check failed (/storage/local, Datastore.AllocateTemplate)` / 403 when downloading an ISO | `Datastore.AllocateTemplate` (+ `Sys.AccessNetwork` on PVE ≥ 8) missing on `PortalAdmin` | `pveum role modify PortalAdmin` (see above). ISO download uses the packer→**admin** token fallback; `PortalPacker` already carries both, so the admin role needs them too when only the admin token is configured |
...
| `node_not_stack_deploy_capable:<node>` / HTTP 400 when deploying a stack (PROJ-76) | **No `PortalTofu` token configured** on the node (not a permission issue) | Stacks apply through OpenTofu and need the dedicated `PortalTofu` token — the viewer/operator/admin/packer tokens are not used. Fill in the **OpenTofu token** under **System Settings → Nodes → \<node\> → Edit** (create the role via `pveum role add PortalTofu …`, see the OpenTofu section). On a cluster, configure it **once** on the portal node that represents the installation |
...
- **Template replication / VM migration (403 on migrate):** cloning a template to another node
  (PROJ-101) and migrating a VM (PROJ-102) use the **admin** token and require **`VM.Migrate`** on
  the `PortalAdmin` role. If the job log shows `Proxmox rejected the action (403)` at the migration
  step, add it: `pveum role modify PortalAdmin --privs "…,VM.Migrate,…"` (full list above).
