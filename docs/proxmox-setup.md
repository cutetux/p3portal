...
| `admin` | `portal-admin@pve` | operator perms + `VM.Allocate`, `VM.Clone`, `VM.Backup`, `VM.Config.{CPU,Memory,Disk,Network,HWType,Options,Cloudinit,CDROM}`, `Datastore.Allocate`, `Datastore.AllocateSpace`, **`Datastore.AllocateTemplate`**, `SDN.Use`, **`SDN.Allocate`**, **`Sys.Audit`**, **`Sys.Modify`**, **`Sys.AccessNetwork`** |
